...
cat >>Dockerfile <<EOF
RUN apt-get install --yes --no-install-recommends git cmake build-essential libssl-dev libxml2-dev libspandsp-dev libsamplerate-dev libcurl3-dev libhiredis-dev librtmp-dev libzrtpcpp-dev libev-dev python-dev libspeex-dev libgsm1-dev libpqxx-dev libmp3lame-dev libopus-dev
